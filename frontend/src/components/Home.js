...
const Home = ({ theme = 'dark' }) => {
  ...
  const flow = [
    { label: "Vague Requirements", color: "bg-sky-500" },
    { label: "Preprocessing", color: "bg-emerald-500" },
    { label: "SRS Making", color: "bg-indigo-500" },
    { label: "Textual Use Cases", color: "bg-purple-500" },
    { label: "Use Case Diagram", color: "bg-amber-500" },
  ];
  ...
  return (
    <div className="animate-fade-in">
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className="text-left">
            ...
            Requirements Engineering
              <span className={`block mt-2 ${isDark ? 'text-sky-200 neon-text' : 'text-indigo-600'}`}>Made Simple</span>
          </h1>
            ...
            <Link 
              to="/input"
                className="group bg-gradient-to-r from-sky-500 to-indigo-500 text-white px-8 py-4 rounded-lg font-semibold transition-all duration-200 flex items-center justify-center space-x-2 shadow-lg hover:shadow-xl hover:scale-105 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
            ...
            <Link 
              to="/results"
                className={`${isDark ? 'bg-slate-800 text-slate-100' : 'bg-slate-100 text-slate-900'} px-8 py-4 rounded-lg font-semibold transition-all duration-200 flex items-center justify-center space-x-2 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2`}
          ...
          <div className="w-full">
            <div className={`p-7 md:p-8 rounded-2xl border ${isDark ? 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white border-slate-700 neon-card' : 'bg-gradient-to-br from-white via-slate-100 to-white text-slate-900 border-slate-200 card-shadow'}`}>
              <div className={`text-sm uppercase tracking-wide font-semibold mb-5 ${isDark ? 'text-sky-200' : 'text-indigo-600'}`}>Pipeline</div>
              <div className="space-y-5">
                {[
                  ...
                  <div key={idx} className="flex items-center space-x-3">
                    <div className={`h-9 w-9 rounded-full bg-gradient-to-br from-sky-500 via-indigo-500 to-fuchsia-500 text-white flex items-center justify-center font-bold text-sm shadow-md`}>
                      {idx + 1}
                    </div>
        ...
        <div className="grid sm:grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {actions.map(({ title, description, cta, link, badge, icon: Icon, accent }, idx) => (
            <Link
              key={idx}
              to={link}
              className={`group relative overflow-hidden rounded-xl p-6 md:p-7 card-shadow hover:shadow-xl hover:-translate-y-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}
            ...
            >
              ...
              <div className="relative flex flex-col h-full space-y-4">
                <div className={`inline-flex items-center space-x-2 text-xs font-semibold px-3 py-1 rounded-full w-fit border ${isDark ? 'text-sky-200 bg-slate-800 border-slate-700' : 'text-indigo-700 bg-indigo-50 border-slate-200'}`}>
                  <Sparkles className="h-3 w-3" aria-hidden="true" />
                  <span>{badge}</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className={`h-11 w-11 rounded-full flex items-center justify-center group-hover:scale-105 transition-transform duration-200 ${isDark ? 'bg-slate-800 text-sky-300' : 'bg-slate-100 text-indigo-600'}`}>
                ...
                <div className={`flex items-center font-semibold group-hover:translate-x-1 transition-transform duration-200 ${isDark ? 'text-sky-300' : 'text-indigo-600'}`}>
                  <span>{cta}</span>
          ...
          {features.map((feature, index) => {
            ...
            return (
              <Link
                key={index}
                to={feature.link}
                className={`group p-6 md:p-8 rounded-xl card-shadow hover:shadow-xl transition-all duration-300 hover:-translate-y-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}
              ...
              >
                <div className="text-center">
                  ...
                  <h3 className={`text-xl font-semibold mb-3 transition-colors duration-200 ${isDark ? 'text-slate-100 group-hover:text-sky-300' : 'text-slate-900 group-hover:text-indigo-600'}`}>
                    {feature.title}
                  </h3>
                  <p className={`${isDark ? 'text-slate-300' : 'text-slate-600'} leading-relaxed`}>
                    {feature.description}
                  </p>
                  <div className={`mt-4 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 ${isDark ? 'text-sky-300' : 'text-indigo-600'}`}>
      ...
      <section id="stats-section" className={`py-12 md:py-16 rounded-xl card-shadow mb-12 border ${isDark ? 'bg-slate-900/90 border-slate-800' : 'bg-white border-slate-200'}`} aria-labelledby="stats-heading">
        ...
        <div className="grid sm:grid-cols-1 md:grid-cols-3 gap-8 text-center">
          {stats.map((stat, index) => (
            ...
            >
              <div className={`text-4xl md:text-5xl font-bold mb-2 tabular-nums ${isDark ? 'text-sky-300' : 'text-indigo-600'}`}>
      ...
      {/* CTA Section */}
      <section className={`text-center py-12 md:py-16 rounded-2xl card-shadow border ${isDark ? 'bg-gradient-to-r from-slate-800 via-indigo-800 to-purple-800 text-white border-slate-800' : 'bg-gradient-to-r from-indigo-100 via-sky-100 to-white text-slate-900 border-slate-200'}`} aria-labelledby="cta-heading">
